...
void Model::configData_v()
{
	glBindVertexArray(_vao);
	glBindBuffer(GL_ARRAY_BUFFER, _vbo);

	glBufferData(GL_ARRAY_BUFFER, _verts.size() * sizeof(value_type), &_verts[0], GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(value_type), (void*)0);
...
void Torus::draw()
{
	...
	if (_showBoundBox) {
		//if (bInit)
		{
			_boundBox.setLWH(_boundLWH);
			_boundBox.init();
			bInit = false;
		}
		_boundBox._spShader->use();
		//glUseProgram(_boundBox._shaderProgID);
		GLuint worldMatLoc, viewMatLoc, projMatLoc;
		worldMatLoc = glGetUniformLocation(_boundBox._spShader->getId(), "world_matrix");
		viewMatLoc = glGetUniformLocation(_boundBox._spShader->getId(), "view_matrix");
		projMatLoc = glGetUniformLocation(_boundBox._spShader->getId(), "proj_matrix");

		F7::Mat4 viewMat = _camera->getViewMatrix();
		F7::Mat4 worldMat = getWorldMatrix();
		F7::Mat4 projMat = _camera->getProjMatrix();
		glUniformMatrix4fv(worldMatLoc, 1, GL_TRUE, worldMat.ptr());
		glUniformMatrix4fv(viewMatLoc, 1, GL_TRUE, viewMat.ptr());
		glUniformMatrix4fv(projMatLoc, 1, GL_TRUE, projMat.ptr());
...
void Torus::init()
{
	_boundRadius = _radius1 + _radius2;
	_boundLWH.x() = _boundLWH.y() = _boundRadius * 2;
	_boundLWH.z() = _radius2 * 2;
...
void Segment::draw()
{
	...
	if (_bUseEBO)
		...
		glDrawArrays(GL_LINES, 0, _vertsNum);
	//glDrawArraysInstanced(GL_TRIANGLES, 0, 36, 100000);
	glBindVertexArray(0);
}
...
void BoundBox::init()
{
	...
	_indices.push_back(0); _indices.push_back(1);
	_indices.push_back(1); _indices.push_back(2);
	_indices.push_back(2); _indices.push_back(3);
	_indices.push_back(3); _indices.push_back(0);

	_indices.push_back(4); _indices.push_back(5);
	_indices.push_back(5); _indices.push_back(6);
	_indices.push_back(6); _indices.push_back(7);
	_indices.push_back(7); _indices.push_back(4);

	_indices.push_back(0); _indices.push_back(4);
	_indices.push_back(1); _indices.push_back(5);
	_indices.push_back(2); _indices.push_back(6);
	_indices.push_back(3); _indices.push_back(7);
...
//ShaderProgram* BoundBox::_spShader = nullptr;

GLuint BoundBox::_shaderProgID = 0;
