...
namespace F7
{
 	...
 	public:
		...
		inline MatMxN_T<value_type, col, row> getTranspose() const {
			MatMxN_T<value_type, col, row> res;
			for (int i = 0; i < row; ++i)
				for (int j = 0; j < col; ++j)
					res[j][i] = _m[i][j];
			return res;
	...
	class MatN_T: public MatMxN_T<T, Tn, Tn>
	{
	public: 
		using value_type = T;
		using base_type = MatMxN_T<T, Tn, Tn>;
		using VecN = Vec_T<value_type, Tn>;
	...
	public:
		...
		static Mat4_T Rotate(const Vec3& angleXYZ) {// TODO: 
			Mat4_T res{ 1 };
...
// 			value_type near, value_type far);
		...
		static Mat4_T Perspective(
			...
			value_type nearValue, value_type farValue)
		{
			Mat4_T res;
			res(1, 1) = 1.f / Math::tan(fovy * 0.5f);
			res(0, 0) = res(1, 1) / aspect;
			value_type tx = 1.f / (nearValue - farValue);
			res(2, 2) = (nearValue + farValue) * tx;
			res(2, 3) = 2 * nearValue * farValue * tx;
			res(3, 2) = -1;
			return res;
		}
	...
	public:
		...
		void makeTrans(const Vec3& site) {
			indentity();
			_m[0][3] = site[0];
			_m[1][3] = site[1];
			_m[2][3] = site[2];
		}
		void makeTrans(value_type e0, value_type e1, value_type e2) {
			indentity();
			_m[0][3] = e0;
			_m[1][3] = e1;
			_m[2][3] = e2;
		...
		inline mat_type operator + (const mat_type& mR) const {
			mat_type res;
			for (int i = 0; i < N; ++i)
				for (int j = 0; j < N; ++j)
					res(i, j) = _m[i][j] + mR._m[i][j];
			return res;
		}
		inline mat_type& operator += (const mat_type& mR) {
			for (int i = 0; i < N; ++i)
				for (int j = 0; j < N; ++j)
					_m[i][j] += mR._m[i][j];
			return *this;
		...
		friend Vec4 operator * (const Vec4& vL, const mat_type& mR) {
			Vec4 res;
			for (int i = 0; i < N; ++i)
				for (int j = 0; j < N; ++j)
					res[i] += mR._m[j][i] * vL[j];
			return res;
		}
		mat_type operator *= (const mat_type& mR) {
			value_type tem;
			Vec4 vTem;
			for (int i = 0; i < N; ++i) {
				vTem.set(0);
...
// 		}
	...
	template <typename T>
	void Quat_T<T>::makeRotate_original(const Vec3_T<value_type>& from, const Vec3_T<value_type>& to)
	{
	...
	template <typename T>
	void Quat_T<T>::slerp(const Quat_T& from, const Quat_T& to, value_type t)
	{
	...
	template <typename T>
	Quat_T<T> Mat4_T<T>::getRotate() const
	{
		Quat_T<value_type> q;
