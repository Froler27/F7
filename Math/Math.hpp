...
namespace F7
{
	...
	bool Quat2Rotate(const T1& quat, T2& vec3Rotate)
	{
		double q0 = quat.w();
		double q1 = quat.x();
		double q2 = quat.y();
		...
		vec3Rotate.x() = (atan2(2 * (q2 * q3 + q0 * q1), (q0 * q0 - q1 * q1 - q2 * q2 + q3 * q3)));
		vec3Rotate.y() = (asin(-2 * (q1 * q3 - q0 * q2)));
		vec3Rotate.z() = (atan2(2 * (q1 * q2 + q0 * q3), (q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3)));
